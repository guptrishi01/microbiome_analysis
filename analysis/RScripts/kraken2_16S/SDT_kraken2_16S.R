...
kraken_input <- paste0(pipeRoot,"/input/RYGB_SDT/Kraken2/")
meta_input <- paste0(pipeRoot,"/input/RYGB_SDT/Metadata/")
...
for (t in taxa){
  ...
  myT<-read.table(paste0(kraken_input,"RYGB_SDT_2026_taxaCount_norm_Log10_",t,".tsv"),sep="\t",header = TRUE,row.names = 1,check.names = FALSE)
  ...
  if(t=="Species"){
    myT2<-cbind(myT,meta)
    write.table(myT2,paste0(output,"SDT_speciesMetadata.txt"),sep="\t",quote = FALSE)
  }
  ...
  write.table(df,paste0(output,t,"_SDT_Kraken2_MixedLinearModelResults.txt"),sep="\t",row.names = FALSE,quote = FALSE)
}
