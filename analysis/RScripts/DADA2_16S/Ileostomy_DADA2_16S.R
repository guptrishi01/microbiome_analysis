...
input <- paste0(pipeRoot,"/",str_subset(dir(pipeRoot), "IleostomyTaxaClass"),"/output/")
...
for (t in taxa){
  ...
  dada2<-read.table(paste0(input,t,"_norm_table_Ileostomy.txt"),sep="\t",header = TRUE,row.names = 1,check.names = FALSE)
  ...
  write.table(df,paste0(output,t,"_Ileostomy_MixedLinearModelResults.txt"),sep="\t",row.names = FALSE,quote = FALSE)
}
